...
const fallbackCopy = (text: string) => {
  ...
  >
    ...
    <td className={`border border-slate-300  [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] max-w-[120px] overflow-x-auto whitespace-nowrap px-2 sm:px-4 py-1 sm:py-2`}>
    {/* {isEdit === key ? (
      <input
        type="text"
        className="w-full h-11 box-border border-2 border-blue-600 px-2" 
        value={keyInput}
  ...
  <td className={`border border-slate-300 max-w-[200px] overflow-x-auto whitespace-nowrap ${isEdit === key ? 'p-0' : 'px-2 sm:px-4 py-1 sm:py-2'}  [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]`}>
    {isEdit === key ? (
      <input
        type="text"
        className="w-full h-11 box-border border-2 border-blue-600 px-2 " // full width & height
        value={valueInput}
    ...
    <td className="border border-slate-300  [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] px-2 sm:px-4 py-1 sm:py-2 flex gap-2 justify-center items-center whitespace-nowrap">
    {isEdit === key ? (
      <button
        onClick={() => doneEdit(key)}
        className="border border-black p-1 rounded-xl text-xs sm:text-sm bg-green-500 w-16 text-white cursor-pointer hover:bg-green-600"
      >
        Done
      </button>
    ) : (
      <>
        <button
          ...
          className="border border-black p-1 rounded-xl text-xs sm:text-sm bg-blue-500 w-16 text-white cursor-pointer hover:bg-blue-600"
        >
          Edit
        </button>
        <button onClick={()=>deleteKey(key)}
          className="border border-black p-1 rounded-xl text-xs sm:text-sm bg-red-500 w-16 text-white cursor-pointer hover:bg-red-600"
        >
          Delete
        </button>
      </>
   ...
   {showNewRow && (
  <tr>
    <td className="border border-slate-300 px-2 sm:px-4 py-1">
      <input
        type="text"
        value={newKey}
    ...
    <td className="border border-slate-300 px-2 sm:px-4 py-1 flex gap-2 justify-center h-14 items-center">
      <button
        onClick={addEnvVar}
        className="bg-green-500 text-white px-2 py-1 cursor-pointer rounded hover:bg-green-600 text-xs sm:text-sm"
      >
        Add
      </button>
      <button
        ...
        className="bg-red-500 text-white px-2 py-1 cursor-pointer rounded hover:bg-red-600 text-xs sm:text-sm"
      >
        Cancel
      </button>
    </td>
